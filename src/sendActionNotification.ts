...
import { getFriendsGroupRef } from './friendshipHelpers';
import { determineUriRef } from './uriHelpers';
import { getDocument } from './DocumentCache';
import { getMyWebId } from './getMyWebId';

export async function determineUriInbox(
  uri: string,
  doc?: string
): Promise<string | null> {
...
export async function determineInboxesToUse(
  recipient: string
): Promise<string[]> {
  const recipientFriendsGroupUrl: string | null = await getFriendsGroupRef(
    recipient,
    false
  );
  let ret: string[] = [];
  ...
  if (recipientFriendsGroupUrl) {
    ...
    const friendsGroupInbox = await determineUriInbox(
      recipientFriendsGroupUrl,
      recipient
    );
