...
async function getContainerDocuments(
  containerUrl: string
...
export function useIncomingFriendRequests(): IncomingFriendRequest[] | null {
  let incomingFriendRequests: IncomingFriendRequest[] | null = null;
  const webId: string | null = useWebId() || null;
  if (webId && !incomingFriendRequests) {
    getIncomingFriendRequests(webId)
...
export async function checkInbox(url: string) {
  ...
  await Promise.all(
    notificationDocs.map(async (doc: TripleDocument) => {
      ...
      if (webId) {
        ...
        await Promise.all(
          filtered.map(async (item: IncomingFriendRequest) => {
            if (item.webId === webId) {
