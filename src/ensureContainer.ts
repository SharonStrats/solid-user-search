...
export async function createAclDoc(
  webId: string,
  resourceUri: string,
  otherAuthMode: string,
  otherAuthPredicate: string,
  otherAuthObject: string,
  recursive = false
) {
  ...
  if (aclDoc) {
    ...
    if (otherAuthObject !== foaf.Agent) {
      // for public, origin isn't checked
      APP_ORIGINS.forEach((origin: string) => {
...
export async function createFriendsGroupAclDoc(
  webId: string,
  friendsGroupUri: string
) {
  return createAclDoc(
    webId,
    friendsGroupUri,
